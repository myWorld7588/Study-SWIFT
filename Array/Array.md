...
**sort** 와 **sorted** 의 차이점에 대해  보통 Swift에서 **함수 이름 동사**에

- **ing**
- **ed**

가 붙는 것들은 **원본을 건드리지 않고, 작업이 완료된 새로운 것을 리턴한다.**

따라서 **sort**는 **동사 그대로이니 원본을 건드는 것**이고, **sorted는 -ed가 붙었으니 원본은 건들지 않고 정렬된 새로운 배열을 리턴**하는 것이다. 

### 이 내용은 sort/sorted 뿐 아니라 **Swift 함수 대부분에 적용**되니까 알아두면 꿀팁임 :)
