...
Swift는, 별도의 작업을 해주지 않더라도 static을 사용해 타입 프로퍼티로 인스턴스를 생성하면, **사용 시점에 초기화(lazy)** 되잖음!? 따라서 **Singleton Instance가 최초 생성되기 전까진 메모리에 올라가지 않고, Dispatch_once도 자동 적용된다고 함!!!!**
